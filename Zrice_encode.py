...
# File paths
input_path = r"C:\Users\PRIYA DIVYA\Desktop\Pattern classif using CA\rice dataset\rice raw dataset.xlsx"
output_path = os.path.expanduser(r"C:\Users\PRIYA DIVYA\Downloads\riceEncode dataset_CSV.xlsx")

# Load the CSV file
df = pd.read_excel(input_path)

# Define lists of continuous attributes
continuous_attributes = ['Area','Perimeter','MajorAxis', 'MinorAxis', 'Eccentricity', 'ConvexArea','Extent']
...
# Display bin ranges for each attribute
for attr, info in encoding_info.items():
    if info['encoding_type'] == 'interval':
        print(f"\nAttribute: {attr}")
        print("Bin Ranges:")
        for bin_index, bin_range in info['interval_dict'].items():
            print(f"  Bin {bin_index}: Range {bin_range}")
