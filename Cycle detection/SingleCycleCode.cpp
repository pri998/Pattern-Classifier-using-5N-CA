...
void CheckCycle(int length , string rule, int &multiCount, ofstream &outfile){
    int SingleCycle = 0;
    ...
    if(SingleCycle > 0){
        ...
        outfile<<"No Such Cycle Detected"<<endl;
    }  
}
...
string s = "00000000000000000000000000000011";
string rule;
sort(s.begin(), s.end());
int count = 0;
    do {
    ...
    outfile<<"The total count of rules permutations for this loop is "<<count<<endl;
    outfile<<"multilength rules count is : "<<multiCount<<endl;
    outfile<<"So, single length rules count is : "<<(count-multiCount)<<endl;
    cout<<"end"<<endl;
//long long eca;
